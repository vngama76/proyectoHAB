  ...
  try {
    ...
    if (tags) {
      for (const tag of tags) {
        ...
        await questionsRepository.addTagToQuestion(resp.id_question, id_tag);
      }
    }
...
async function getQuestionsByUserId(req, res, next) {
  try {
    ...
    const question = await questionsRepository.findQuestionByUserId(id_user);
...
async function getQuestions(req, res, next) {
  try {
    ...
    let questionTags = await questionsRepository.findQuestionsByTag(wordsSplit);
...
async function getQuestionsByTags(req, res, next) {
  try {
    const { words } = req.params;
    const wordsSplit = words.split(' ');
    let questions = await questionsRepository.findQuestionsByTag(wordsSplit);
...
async function getRandomQuestionLanding(req, res, next) {
  try {
    const question = await questionsRepository.findRandomQuestionLanding();
